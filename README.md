...
1. 下一交易日开盘价为模拟买入价；一字涨停、停牌或没有有效开盘价记为未成交；
2. 再下一交易日开盘价为标准卖出价，当日最高价用于计算最高可能收益；
3. 退出日一字跌停、停牌或没有有效开盘价时顺延到首个可卖交易日，不排除最差样本；
4. 第三日开盘高于买入价为成功，低于为失败，持平单列且不进入成功率分母；
5. 标准收益和最高可能收益均按每笔等金额做算术平均，不使用复利；
6. 同一股票同一信号交易日只保留一笔，来自多个题材时合并来源；不同信号日可重复测试。

结算使用Tushare `daily`正式日线，不使用某个盘中快照代替。服务启动、08:35和17:10会检查待结算日期并自动回补；数据失败会进入系统预警。最高可能收益是当日最高价对应的理论上限，不代表真实可成交收益。详细口径见[测试票池说明](docs/BACKTEST.md)。
