...
- 正式题材全部成分股使用Tushare盘后资金流及三源方向验证，保留历史与成分股快照。
...
正式题材全部有效成分股盘后使用Tushare `moneyflow`，以20–100万元大单与≥100万元超大单的买卖差额之和计算主力净额。东方财富、同花顺通过 `moneyflow_dc`、`moneyflow_ths` 获取盘后结果。盘中东方财富资金分时接口只有在返回有效同日数据后才写入数值。

详细规则、迁移与验证见[资金流说明](docs/FUND_FLOW.md)。
